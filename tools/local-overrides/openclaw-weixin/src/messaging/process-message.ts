...
import { maybeHandleJiuyanWeixinDirectOps } from "./jiuyan-data-ops.js";
...
export async function processOneMessage(
  ...
  if (
    await maybeHandleJiuyanWeixinDirectOps({
    ...
    logger.info(`weixin Jiuyan direct handler completed to=${ctx.To}`);
    return;
  }
  ...
  if (isJiuyanDataOpsFile(finalized.MediaPath) && textBody.trim().length === 0) {
    ...
    logger.info(`weixin intercepted standalone Jiuyan file message to=${ctx.To}`);
    return;
  }
